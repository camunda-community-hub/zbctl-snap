...
set -euxo pipefail
wget "https://github.com/camunda/zeebe/releases/download/$1/camunda-zeebe-$1.tar.gz.sha1sum"
sed -i "s/^version: '[^']*'/version: '$1'/" snap/snapcraft.yaml
checksum=$(head -c 40 "camunda-zeebe-$1.tar.gz.sha1sum")
sed -i "s#    source-checksum: sha1/.*#    source-checksum: sha1/$checksum#" snap/snapcraft.yaml
rm "camunda-zeebe-$1.tar.gz.sha1sum"
